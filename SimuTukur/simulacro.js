...
document.addEventListener("DOMContentLoaded", () => {
      ...
      (async () => {
        ...
        try {
          ...
          await _supabase.functions.invoke("correo-simulacro", {
            body: {
              tutor_nombre: datos.tutor,
              alumno_nombre: datos.alumno,
              correo_destino: datos.correo,
              examen_elegido: datos.examen,
              horario_elegido: datos.dia_elegido,
            },
          });
          ...
          alert(
            "Tu pago se procesó, pero hubo un error de conexión al guardar tu lugar. Toma captura de tu recibo de Stripe y contáctanos al WhatsApp de soporte.",
          );
        }
      })();
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      const nombreExamenFiltro =
        selectExamen.options[selectExamen.selectedIndex].dataset.nombreExamen;
      const dia = "16 de Mayo";

      const codigoPadrinoInput = document
        .getElementById("codigoPadrino")
        .value.trim()
        .toUpperCase();
      const tipoEscuela = document.getElementById("tipoEscuela").value; // <-- TE FALTABA ESTE
      const nombreEscuela = document
        .getElementById("nombreEscuela")
        .value.trim();
      const tomaCurso = document.getElementById("tomaCurso").value;
      const nombreEscuelaCurso = document
        .getElementById("nombreEscuelaCurso")
      ...
      try {
        ...
        if (codigoPadrinoInput !== "") {
          ...
          if (codigoBD.usos_actuales >= codigoBD.limite_usos) {
            alert(
              "⚠️ Los lugares gratuitos para este código se han agotado. Puedes registrarte sin código cubriendo la cuota de recuperación.",
            );
            // Restauramos el botón respetando su diseño actual (Pago o Beca)
            document.getElementById("iconoBoton").className =
              window.modoRegistro === "beca"
...
async function cargarExamenesBD() {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      select.innerHTML =
        '<option value="" disabled selected class="text-gray-400">No hay exámenes disponibles</option>';
      return;
    }

    // Filtramos para jalar ECOEMS, UNAM e IPN
    const examenesHabilitados = data.filter((ex) =>
      ["ECOEMS", "UNAM", "IPN"].includes(ex.institucion),
    );
    ...
    select.innerHTML =
      '<option value="" disabled selected class="text-gray-400">Selecciona tu examen...</option>';
    ...
    select.innerHTML =
      '<option value="" disabled selected class="text-red-400">Error de red. Recarga la página.</option>';
  }
}
...
async function llamarApiStripe() {
  ...
  try {
    ...
    alert(
      "Tuvimos un problema al conectar con el banco. Intenta en un momento.",
    );
...
async function validarCodigoPadrino() {
  ...
  const { data: padrinoValido } = await _supabase
    .from("codigos_padrinos")
    .select("*")
    .eq("codigo", codigo)
    .eq("activo", true)
  ...
  if (padrinoValido) {
    // 🔥 2. MOSTRAR MENSAJE GRANDE CON ANIMACIÓN
    contenedor.classList.remove("hidden");
    setTimeout(() => {
      contenedor.classList.remove("opacity-0", "scale-95");
      contenedor.classList.add("opacity-100", "scale-100");
    }, 10);
