...
async function iniciarSesion() {
    ...
    try {
        const respuesta = await fetch(`${supabaseUrl}/functions/v1/academia_auth_crm_vendedores`, {
            method: 'POST',
...
async function cargarProspectos() {
    ...
    try {
        const respuesta = await fetch(`${supabaseUrl}/functions/v1/academia_prospectos_escuelas`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${supabaseKey}` },
    ...
    try {
        ...
        documentosRH.forEach(doc => {
            ...
            if (estatus === 'Validado' || estatus === 'En Revisión') {
                ...
                controles = `
                    <input type="file" id="file_${doc.id}" accept=".pdf, .jpg, .png" style="font-size: 11px; color: #334155; width: 100%; margin-bottom: 8px;">
