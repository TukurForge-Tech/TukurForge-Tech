...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (accion === 'listar_prospectos') {
      const { data, error } = await supabase.from('academia_crm_prospectos').select('*').eq('vendedor_id', vendedor_id).order('created_at', { ascending: false })
    ...
    if (accion === 'guardar_prospecto') {
      const { error } = await supabase.from('academia_crm_prospectos').insert([payload])
      if (error) throw error
    ...
    if (accion === 'actualizar_estatus') {
      ...
      const { error } = await supabase.from('academia_crm_prospectos').update({ estatus: nuevo_estatus }).eq('id', id_prospecto)
      if (error) throw error
      return new Response(JSON.stringify({ success: true }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    if (accion === 'cargar_expediente') {
      let { data, error } = await supabase.from('academia_onboarding_trabajadores').select('*').eq('trabajador_id', vendedor_id).single()
      if (!data) {
         // Si no existe, creamos su fila vacía
         const { data: newData, error: errInsert } = await supabase.from('academia_onboarding_trabajadores').insert([{ trabajador_id: vendedor_id }]).select().single()
    ...
    if (accion === 'actualizar_documento_rh') {
      ...
      const { error } = await supabase.from('academia_onboarding_trabajadores').update(updateData).eq('trabajador_id', vendedor_id)
      if (error) throw error
      return new Response(JSON.stringify({ success: true }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
